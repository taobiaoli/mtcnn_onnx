...
from utils.mtcnn_ykx import detect


WINDOW_NAME = 'ONNXMtcnnDemo'
...
def loop_and_detect(cam, minsize):
    ...
    while True:
        ...
        img = cam.read()
        if img is not None:
            boxes,landmarks = detect(img, minsize=minsize)
            
            print('{} face(s) found'.format(len(boxes)))
            img = show_faces(img, boxes, landmarks)
...
def main():
    ...
    open_window(
        WINDOW_NAME, 'Camera ONNX MTCNN Demo ',
